...
export const seedCustomCalendarForYear = async year => {
  ...
  try {
    // Check if the calendar for the specified year already exists
    const existingCalendar = await CustomCalendarModel.findOne({ year })

    if (!existingCalendar) {
      // Insert a new calendar document for the specified year
      const newCalendar = new CustomCalendarModel({
        year,
        months: customCalendar
      })

      await newCalendar.save()
      console.log(`Custom calendar for year ${year} seeded successfully!`)
    } else {
      console.log(`Custom calendar for year ${year} already exists!`)
    }
  } catch (error) {
    console.error('Error seeding custom calendar:', error)
  }
}

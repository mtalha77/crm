...
import { WriterFormType, writerDefaultValues } from 'src/interfaces/forms.interface'
import { Department } from 'src/shared/enums/Department.enum'
import { writerYupSchema } from 'src/yupSchemas/writerYupSchema'
import { mapResponseForWriter } from 'src/utils/mapResponseForWriter'
import WriterForm from '../newTicketForm/Departments/Writer'

const schema = writerYupSchema

const WriterFormComponent = () => {
  ...
  const defaultValues = async () => {
    ...
    try {
      ...
      return mapResponseForWriter(res.data.payload.ticket)
    } catch (error: any) {
  ...
  const onSubmit = async (data: WriterFormType) => {
    const { business, saleDepart, ticketDetails, writerFormTypeDetails } = data
    ...
    if (update) {
      ...
      await axios
        ...
        .then(() => {
          ...
          methods.reset(writerDefaultValues)
        })
        .catch(error => {
          console.error('Error:', error)
          toast.error(error?.response?.data || 'Something went wrong')
    ...
    <>
      <FormProvider {...methods}>
        <form noValidate autoComplete='off' onSubmit={methods.handleSubmit(onSubmit as any)}>
          {apiLoading ? <Spinner /> : <WriterForm update={update} />}
        </form>
      </FormProvider>
    </>
  )
...
export default WriterFormComponent
